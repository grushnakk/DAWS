...
public class Dicto {
	...
	public StateMachine build(Environment env) {
		...
		smBuilder.state(STATE_VAR_TYPE)
						...
						.accept(typeAcceptor(env).region(REGION_TYPE))
						.suggest(new VarTypeSuggestor())
		...
		smBuilder.state(STATE_ARG_NAME)
						...
						.accept(idAcceptor.region(REGION_ARG_NAME).chain(optionalWhitespace(), string(":")))
						.suggest(new ArgNameSuggestor())
		...
		smBuilder.state(STATE_AFTER_ARG)
						.pathTo(STATE_START)
						.accept(new MultiStringAcceptor("\n", "\r"))
		...
		smBuilder.state(STATE_ID_AFTER_ONLY)
						...
						.accept(idAcceptor.region(REGION_ID))
						.suggest(new VariableSuggestor())
		...
		smBuilder.state(STATE_ONLY_MULTI_SUBJECT)
						.pathTo(STATE_ONLY_CAN)
						.accept(idAcceptor.region(REGION_ID))
						.suggest(new VariableOfSameTypeSuggestor())
		...
		smBuilder.state(STATE_RULE_PREDICATE)
						.pathTo(STATE_RULE)
						.accept(new MultiStringAcceptor(new PredicateSuggestor().suggestions(env)).region(REGION_PREDICATE))
						.suggest(new PredicateSuggestor())
		...
		smBuilder.state(STATE_RULE)
						...
						.accept(ruleAcceptor(env).region(REGION_RULE))
						.suggest(new RuleSuggestor())
		...
		smBuilder.state(STATE_RULE_PARAMETER)
						...
						.accept(string("\"").chain(negRange("\"").repeat(), string("\"")))
						.complete();
		smBuilder.state(STATE_RULE_PARAMETER)
						.pathTo(STATE_RULE_AFTER_PARAMETER)
						.accept(idAcceptor.region(REGION_PARAMETER))
						.suggest(new VariableSuggestor())
